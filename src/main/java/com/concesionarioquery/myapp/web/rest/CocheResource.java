...
/**
 * REST controller for managing {@link com.concesionarioquery.myapp.domain.Coche}.
 */
@RestController
@RequestMapping("/api")
public class CocheResource {
    ...
    public ResponseEntity<CocheDTO> updateCoche(
        ...
        @Valid @RequestBody CocheDTO cocheDTO
    ...
    public ResponseEntity<CocheDTO> partialUpdateCoche(
        @PathVariable(value = "id", required = false) final Long id,
        @NotNull @RequestBody CocheDTO cocheDTO
    ) throws URISyntaxException {
    ...
    public ResponseEntity<List<CocheDTO>> getAllCochesPorBusqueda(Pageable pageable, @RequestParam(required = false, defaultValue="" ) String filtro)
    throws InterruptedException, URISyntaxException{
        log.debug("REST request to get a page of Coches");
        filtro = !filtro.equals("undefined") ? filtro : "%";
        Page<CocheDTO> page = cocheService.findAllByParam(pageable, filtro);
        filtro = !filtro.equals("undefined") ? filtro : "%";
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of coches in body.
     */
    @GetMapping("/coches/expuestos/{expo}")
    public ResponseEntity<List<CocheDTO>> getAllExpuestos(@PathVariable Boolean expo, Pageable pageable) {
        log.debug("REST request to get a page of Coches");
        Page<CocheDTO> page = cocheService.cochesExpuestos(expo, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
    /**
